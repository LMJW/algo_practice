...
B = int(1e9)
N = 30
CORRECT_GUESS_LIST = [random.randint(A, B) for i in range(1000)]
...
def CheckSubprocessExit(p, case_id):
    ...
    if p.poll() is not None:
        print("Your code exited early, in the middle of Case #{}.".format(
            case_id))
        PrintSubprocessResults(p)
        sys.exit(-1)
...
def WrongAnswerExit(p, case_id, error_msg):
    ...
    try:
        ...
        print(
            "Failed to print WRONG_ANSWER because your code finished already.")
    WaitForSubprocess(p)
    sys.exit(-1)
    ...
    for _ in range(N):
        ...
        try:
            ...
            if user_input == "":
                ...
                exit_msg = (
                    "Failed to read the guess. Expected an integer ending with "
        ...
        if (q <= A) or (q > B):
            WrongAnswerExit(p, i + 1,
                            "Your guess, {}, is out of range!".format(q))
        if q == answer:
